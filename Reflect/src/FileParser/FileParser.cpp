...
void FileParser::ParseDirectory(const std::string& directory) {
  ...
  for (const auto& f :
    ...
    if ((f.is_regular_file() || f.is_character_file()) &&
      ...
      data.FileName = f.path().filename().u8string().substr(
          0, f.path().filename().u8string().find_last_of('.'));
  ...
  for (auto const& fileToRemove : m_filesToRemove) {
    auto itr = std::find_if(m_filesParsed.begin(), m_filesParsed.end(),
                            [fileToRemove](FileParsedData const& data) {
                              return fileToRemove == data.FileName;
                            });
    assert(
        itr != m_filesParsed.end() &&
        "[FileParser::ParseDirectory] Remove file to parse dose not exists.");
...
bool FileParser::CheckExtension(const std::string& filePath,
...
bool FileParser::ParseFile(FileParsedData& fileData) {
  ...
  while (
      ReflectContainerHeader(fileData, RefectStructKey, ReflectType::Struct) ||
      ReflectContainerHeader(fileData, RefectClassKey, ReflectType::Class)) {
...
bool FileParser::ReflectContainerHeader(FileParsedData& fileData,
                                        const std::string& keyword,
                                        const ReflectType type) {
  // Check if we can reflect this class/struct.
  int reflectStart =
      static_cast<int>(fileData.Data.find(keyword, fileData.Cursor));
  ...
  while (fileData.Data.at(fileData.Cursor) != ':' &&
         fileData.Data.at(fileData.Cursor) != '{' &&
         fileData.Data.at(fileData.Cursor) != '\n') {
...
void FileParser::ReflectContainer(FileParsedData& fileData) {
  ...
  // First find out which it is and verify that we are inheriting from
  // "ReflectObject".
  std::stack<char> bracketStack;
  ReflectContainerData& conatinerData = fileData.ReflectData.back();

  int generatedBodyLine = static_cast<int>(fileData.Data.find(
      ReflectGeneratedBodykey, fileData.GeneratedBodyLineOffset));
  assert(generatedBodyLine != -1 &&
         "[FileParser::ReflectContainer] 'REFLECT_GENERATED_BODY()' is missing "
         "from a container.");
  fileData.GeneratedBodyLineOffset =
      generatedBodyLine + static_cast<int>(strlen(ReflectGeneratedBodykey));
  conatinerData.ReflectGenerateBodyLine =
      CountNumberOfSinceTop(fileData, generatedBodyLine, '\n') + 1;

  while (true) {
    int reflectStart =
        static_cast<int>(fileData.Data.find(PropertyKey, fileData.Cursor));
    if (reflectStart == static_cast<int>(std::string::npos) ||
        reflectStart > endOfContainerCursor) {
      // There are no more properties to reflect or we have found a new
      // container to reflect.
      break;
    }
    ...
    if (c == ';') {
      ...
      memberData.ReflectMemberType =
          type.back() == '*' || type.back() == '&'
              ? (type.back() == '*' ? ReflectMemberType::Pointer
                                    : ReflectMemberType::Reference)
              : ReflectMemberType::Value;
      ...
      assert(false &&
             "[FileParser::ParseFile] Unknown reflect type. This must be a "
             "member variable or function. Make sure ')' or ';' is used before "
             "a new line.");
    }
...
char FileParser::FindNextChar(FileParsedData& fileData,
                              const std::vector<char>& ingoreChars) {
  while (std::find(ingoreChars.begin(), ingoreChars.end(),
                   fileData.Data[fileData.Cursor]) != ingoreChars.end()) {
    ++fileData.Cursor;
  }
...
std::tuple<std::string, std::string, bool> FileParser::ReflectTypeAndName(
    FileParsedData& fileData, const std::vector<char>& endOfLineCharacters) {
  std::string type;
  ...
  while (true) {
    ...
    if (RefectCheckForEndOfLine(fileData) ||
        (std::find(endOfLineCharacters.begin(), endOfLineCharacters.end(), c) !=
         endOfLineCharacters.end())) {
      if (!typeFound) {
        if (!type.empty()) {
          typeFound = true;
...
void FileParser::CheckForConst(FileParsedData& fileData, std::string& type,
                               bool& typeFound, bool& isConst) {
  const int len = 6;
  std::string tmp;
  if (isConst) {
    return;
...
int FileParser::CountNumberOfSinceTop(const FileParsedData& fileData,
                                      int cursorStart, const char& character) {
  int count = 0;
  while (cursorStart > 0) {
